...
class Types_Taxes_Order {
	...
	public function order_install() {
		...
		update_option( 'kcn_order_install', 1 );
	}
	...
	public function admin_menu() {
		...
		add_options_page(
			__( 'Posts & Taxonomies Sort Order', 'kc-network' ),
			__( 'Sort Order', 'kc-network' ),
			'manage_options',
			'sort-order-settings',
			[ $this, 'admin_page' ]
		);
	...
	public function admin_page() {
		require KCN_PATH . 'views/backend/forms/settings-page-posts-order.php';
	}
	...
	public function load_script_css() {
		...
		if ( $this->_check_load_script_css() ) {
			...
			wp_enqueue_script( 'post-tax-order', KCN_URL . 'assets/js/post-tax-order' . $suffix . '.js', [ 'jquery' ], null, true );
	...
	public function update_options() {
		...
		if ( ! isset( $_POST['kcn_posts_order_submit'] ) ) {
			return false;
		}

		check_admin_referer( 'kcn_posts_order_nonce' );

		$input_options            = [];
		$input_options['objects'] = isset( $_POST['objects'] ) ? $_POST['objects'] : '';
		$input_options['tags']    = isset( $_POST['tags'] ) ? $_POST['tags'] : '';

		update_option( 'kcn_order_options', $input_options );
	...
	public function get_order_options_objects() {
		...
		if ( $kcn_order_options = get_option( 'kcn_order_options' ) ) {
			$kcn_order_options = get_option( 'kcn_order_options' );
		} else {
			$kcn_order_options = [];
		}

		if ( isset( $kcn_order_options['objects'] ) && is_array( $kcn_order_options['objects'] ) ) {
			$objects = $kcn_order_options['objects'];
		} else {
			$objects = [];
		}
	...
	public function get_order_options_tags() {
		...
		if ( isset( $kcn_order_options['tags'] ) && is_array( $kcn_order_options['tags'] ) ) {
			$tags = $kcn_order_options['tags'];
		} else {
			$tags = [];
		}
